...
GCS_INPUT_BUCKET = '<gcs-input-bucket-name>'                            # Ex: GCS_INPUT_BUCKET = 'invoice-drop' 
GCS_INPUT_PREFIX = ''                                                   # Prefix to be detected for inputs files such as dates, sender etc., 
output_bucket="<gcs-output-bucket-name>"                                # Ex: GCS_INPUT_BUCKET = 'invoice-drop'
GCS_OUTPUT_URI = f'gs://{output_bucket}'                                # Output bucket will store parsed invoice CSVs and JSON results
GCS_OUTPUT_URI_PREFIX = 'results'
TIMEOUT = 140                                                           # Timeout to accomodate invoice parsing and conversion to JSON
bq_dataset = "<bq-dataset-name>"                                        # Ex: bq_dataset = "document-ai" 
invoice_table = "<bq-table-name>"                                       # Ex: invoice_table = "invoice_data" >> Stores invoice specific data viz., sender, receiver and amounts based on transformations
inventory_table = "<bq-table-name>"                                     # Ex: invoice_table = "inventory_sold" >> Stores line items  data as inventory sold based on transformations


#---------------------------------------------------------------------------------------------------------------------

def upload_pd_as_csv_to_gcs(bucket_name, pdframe, out_file_name):
    print(f" * Uploading Invoice data as CSV to output bucket >> {output_bucket}")
    storage_client = storage.Client()
    bucket = storage_client.get_bucket(bucket_name)
    bucket.blob(out_file_name + '.csv').upload_from_string(pdframe.to_csv(index=False), 'text/csv')
    print(f" ** ...Upload completed. Creating schema for {out_file_name} - Bigquery ingest...")


def load_csv_to_bq(import_file, schema):
    print(f" * Create {import_file} in Bigquery...")
    ...
    print(f' ** ...{destination_table.num_rows} rows now available in table {import_file}')
...
def process_document_sample():
    ...
    for i, blob in enumerate(blob_list):
        ...
        if ".json" in blob.name:
            ...
            print(f'Fetched file {i} >> {blob}')
            print("Downloading blob as bytes for proessing...")
...
def transform_data(df: dict):
    print(" * Beginning data normalization and transformation...")
    ...
    print(" ** ...Data tranformation completed.")
